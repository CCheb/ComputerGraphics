...
int main()
{
    ...
    // now for each vertex we specify its normal vector which is a vector that is
    // perpendicular to the surface that the vertex takes part of. You can visualsize
    // by imagining unit vectors that are at each of the corners facing their respective
    // directions.
    // ------------------------------------------------------------------
    float vertices[] = {
        // for example this is the back surface of the cube
        // we can tell since the normal is facing straight ahead in the
        ...
        -0.5f, -0.5f,  0.5f,  0.0f, -1.0f,  0.0f,
        -0.5f, -0.5f, -0.5f,  0.0f, -1.0f,  0.0f,
        // 6 total faces each with their own normals
        -0.5f,  0.5f, -0.5f,  0.0f,  1.0f,  0.0f,
         0.5f,  0.5f, -0.5f,  0.0f,  1.0f,  0.0f,
         0.5f,  0.5f,  0.5f,  0.0f,  1.0f,  0.0f,
         0.5f,  0.5f,  0.5f,  0.0f,  1.0f,  0.0f,
    ...
    // we dont work with the normals since the light cube is in its own fragment shader that does very basic 
    // lighting.
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 6 * sizeof(float), (void*)0);
    glEnableVertexAttribArray(0);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glm::mat4 model = glm::mat4(1.0f);  // essentially at the origin. camera is already in (0.0,0.0,3.0)
        ...
        {
            // make sure to times by delta or the cube will go lightspeed
            lightPos.x += lightCubeSin * deltaTime;
            lightPos.y += lightCubeCos * deltaTime;
        }
        else
        {
            lightPos.x += lightCubeSin * deltaTime;
        ...
        model = glm::mat4(1.0f);
        // could create an orbit by translating and the rotating 
       // model = glm::rotate(model, float(glfwGetTime())*deltaTime, glm::vec3(0.0f,1.0f,0.0f));
